---
title: "Predicting Whether Football Games Are Worth Watching"
date: 2019-05-21
tags: [projects, machine learning, analytics, classification]
mathjax: true
classes: wide
---
...
I am a passionate football (soccer[^1]) player and a semi-passionate football viewer. Whilst I do enjoy watching football games with friends, many times that I watch a football game at home to relax I get bored or distracted. Whilst I love playing football, I only find some games enjoyable to watch. Usually, I try to hedge my bets with football viewing and watch the two most highly placed teams in the English Premier League (EPL) table on the date of the fixture. However, I have lost count of the number of times I have stayed awake past my bedtime to watch two highly regarded teams play a boring 0-0 draw. So, the problem at hand for this project is:
...
* The teams that have been relegated and promoted the most in order are: West Brom, Birmingham, Hull, Norwich, Burnley, Middlesbrough and Sunderland.
...
It was found that 33.8% of games fulfill one of these criteria. I was somewhat surprised by that because I felt the criteria were fairly strong and would be rare. Evidently, the way I have been selecting games up to now is subpar!

Next, is to decide what statistics will be fed into the model to determine whether a future game will be worth watching or not. Of course, these must be available before the game takes place. I thought about trying to use the betting statistics which are given as home win odds, away win odds and draw odds. But these will give more of an impression of whether a game has gone according to plan or is an upset. Whilst this does impact a games excitement, these concepts have not been implemented in my criteria.
...
Whilst the database had the statistics to compute these averages it was quite the challenge to find them. The code can be viewed in the Jupyter Notebook [here in code cell number 31](https://github.com/sjhatfield/worthwatching/blob/master/EPL_Wrangling_Analysis.ipynb). It was very satisfying when it finally worked!
...
[Here is a post briefly explaining the idea behind the $$k$$-Nearest Neighbors classifier.](https://sjhatfield.github.io/KNN/)
...
I decided to implement the $$k$$-nearest neighbors model myself just using [Pandas](https://pandas.pydata.org/) and [NumPy](https://www.numpy.org/) as a fun challenge. The first step for the model to execute is to normalize all the columns of the DataFrame. This ensures that one column may not have a big impact on the classification due to its magnitude being vastly different. This doesn't apply so much to my data but imagine one column being salary and one being number of years employed at a company. The salary column would be many orders of magnitude larger than the years and would stop the years having much of an impact on the output.

Normalizing uses built-in functions of Pandas and the concept of standardizing a variable which I have taught many times to my High School mathematics students.
...
Once we have the validation set, we split it into 10 subsets. Then, taking each of the 10 parts in turn, we predict the 10% of the validation set using the remaining 90%. This is done 10 times keeping a running total of the number of correct predictions. Then we have an accuracy for the current value of $$k$$. We repeat this for the values of $$k$$ we want to assess and the one with the highest accuracy is selected.

The final step, is to predict the testing data using all the validation data and the chosen value of $$k$$, to get a final accuracy. This last step ensures that any bias in the validation data which the $$k$$-value may have fitted has been removed from our final accuracy measure.
...
**This gave a final accuracy of 65.4%**[^2]. This may sound low but we are in a sense, predicting the outcome of a sporting event which has a huge element of chance involved in the outcome. If predicting sporting events was easy, then a lot of people would make money gambling on them!

As can be seen in the code snippet above I also found values from the confusion matrix. If I truly value my time, I should be concerned with false positives. In this situation the model will tell me to watch a game but it will actually be dull. Sadly, the values from the confusion matrix were not great:
...
These values show that in roughly 30 weekends of EPL football, at most 11 will have a game that is predicted to be exciting and will actually be exciting. What is more concerning are the 12 false positives. This is the situation I find myself in often, of watching a game expecting it be exciting but being bored. We can also see many false negatives, games that were exciting but the model would not have predicted to be so. If we consider the case where the model tells us to watch a game, there is an 11 out of 23 chance of the game being worth watching. This is very close to my desired outcome of 50% accuracy!

So, all in all, whilst the model does have good accuracy, it is not the most useful in practice for predicting which games to watch.
...
The same process was employed of validation and then testing but using Scikit-Learns built in prediction function. I also made use of its confusion_matrix function in the metrics library.

We saw very similar results as my own implemenation, except for taking roughly 2000 times less time. Very similar results we obtained from the confusion matrix distributed over the much larger full 20% testing data.

## What Next?

Clearly, I have not reached my ideal outcome which was, *"Given the list of fixtures taking place next weekend and the current form of all the teams in the league, predict which will games will be exciting to watch"*. I am not satisfied with the predictions obtained using $$k$$-nearest neighbors. My plan is to instead make predictions using **logistic regression**. This is going to be more appropriate as the output of a logistic regression model will be a probability of a game being exciting. I will then be able to order the upcoming games based upon likelihood of being exciting. Then at least I will always have a recommendation for all weekends. This will be one of my next posts.

Thanks for reading, if you have any thoughts/suggestions/criticisms/spelling errors/bugs/ideas please leave a comment below!
...
[^2]: It is probably worth pointing out that with an average of 33% of games being worth watching, the classifier which always predicts a game will be dull will be correct 67% of the time. However, with that predictor we would never watch any football! The values in the confusion matrix prove that my model is not working that way.