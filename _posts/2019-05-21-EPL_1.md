...
These values show that in roughly 30 weekends of EPL football, at most 11 will have a game that is predicted to be exciting and will actually be exciting. What is more concerning are the 12 false positives. This is the siutation I find myself in often, of watching a game expecting it be exciting but being bored. We can also see many false negatives, games that were exciting but the model would not have predicted to be so.

So all in all, whilst the model does have fairly high accuracy, it is not the most useful in practice for predicting which games to watch.
...
The same process was employed of validation and then testing but using Scikit-Learns built in prediction function. I also made use of it's confusion_matrix function in the metrics library.

We saw almost exactly the same results as my own implemention, except for taking roughly 2000 times less time. Very similar results we obtained from the confusion matrix distributed over the much larger full 20% testing data.

## What Next?

Clearly I have not reached my ideal outcome which was *"Given the list of fixtures taking place next weekend and the current form of all the teams in the league, predict which will games will be exciting to watch"*. I am not satisfied with the current predicitons obtained using k-nearest neighbors. My plan is to instead make predictions using **logistic regression**. This is going to be more appropriate as the output of a logistic regression model will be a probability of a game being excited. I will be able to use this to order the upcoming games based upon likelihood of being exciting. Then at least I will always have a recommendation for all weekends. This will be my next blogpost.
